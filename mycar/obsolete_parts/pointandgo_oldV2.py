...
class PointAndGo(object):
    ...
    def calculate_point_to_nav(self):
        ...
        delta_x, delta_y = CoordinationTransformer.transform(self.CoordinationTransformer_ref,
                                                             self.left_click_location, self.is_simulated, self.depth_locations)
    ...
    def run_threaded(self, sim_img, rs_cam, info, tracking_info, depth_locations):
        """main part loop"""
        ...
        if info:
            ...
            if self.out_img is not None:
                ...
                # cv2.createTrackbar('Left/Right Camera', 'DonkeyCar VideoFeed', 0, 1, self.nothing)
...
class PID:
    ...
    def __init__(self):
        ...
        self.throttle_cap = 0.6     # values bigger than that have caused undesired behavior
        # self.vel = 0.0
    ...
    def pid(self, velocity, des_velocity):
        ...
        # self.vel = self.vel + acc * self.delta_time
        # isDrivingForward = -1 if self.vel < -0.005 and des_velocity < -0.005 else 1
        # print("error:", error, "d_error:", self.prev_error, "integral error:", self.error_sum)
        global f
        f.write(f'{acc},{error}\n')
        return acc  # , isDrivingForward
    ...
    def calculate_pid(self, error):
        if abs(error) < self.tol:
            ...
            self.prev_error = 0
            return 0
        control_signal = self.Kp*error + self.Kd*(error-self.prev_error)/self.delta_time + self.Ki*self.error_sum
...
class CoordinationTransformer:
    ...
    def transform(self, pixel, is_simulated, depth_locations):
        ...
        if is_simulated:
            ...
            y = -depth_locations[resolution[0]*pixel[1]+pixel[0]][0]+0.05
