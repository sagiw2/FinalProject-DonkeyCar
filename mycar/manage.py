...
# from pointandgo import PointAndGo
from donkey_car_test import PointAndGo
from copycat import copy
from copycat import paste
# from realsense_camera import RealsenseCam
from T265_donkey import FullDataReader
from D435_donkey2 import RealSense435
from D435_donkey import RealSense435_pub
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[]):
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        # V.add(RealSense435(), outputs=['cam/image_array', 'pixle_3d_location'], threaded=True)
        # V.add(FullDataReader(image_output=False, debug=False), outputs=['tracking_info'], threaded=True)
        # V.add(copy(),inputs=['user/angle', 'user/throttle'], outputs=[], threaded = True)
        # V.add(paste(), inputs=[], outputs=['user/angle', 'user/throttle'], threaded=False)
        ...
        cam = RealSense435_pub()
        V.add(cam, inputs=[],
              outputs=['Cam/image_array', 'cam/depth_array'],threaded=True)

    elif use_pointandgo:

        # T265
        V.add(FullDataReader(image_output=False, debug=False), outputs=['tracking_info', 'yaw_angle'], threaded=True)
        # D435
        V.add(RealSense435(), outputs=['cam/rs_image', 'pixle_3d_location'], threaded=True)

        ctr = PointAndGo()
        V.add(ctr, inputs=[None, 'cam/rs_image', 'info', 'tracking_info','yaw_angle', 'pixle_3d_location'], outputs=['user/angle', 'user/throttle', 'user/mode', 'recording'], threaded=True)
